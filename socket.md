...
### 사장과 고객의 id가 중복될 수 밖에 없는 상황이라면??
만일 그렇지 못하다면 쿠키의 accessToken에 사장인지 고객인지 구분할 수 있는 정보를 id와 함께 담아둡니다.  
가령 res.locals.user.kind = "Client" 또는 res.locals.user.kind = "Owner" 와 같이 담을 수 있다면 좋겠죠.  
그렇게 하면 socket을 통해 쿠키를 가져올 때 jwt verify를 거쳐 해당 소켓의 주인이 Client인지 Owner인지 구분할 수 있고, 이를 활용할 수 있을 것입니다.  
우선은 아래 코드는 id가 완벽히 분리되어있다는 것을 전제로 합니다.
